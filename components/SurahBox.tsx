...
const SurahBox: FC<SurahBoxProps> = ({
  ...
  return (
    <div
      key={number}
      className="flex w-full  max-w-[350px] justify-between items-center p-4 py-8 bg-muted rounded-lg border hover:border-primary hover:bg-gradient-to-r hover:from-primary/10 group"
    >
      <Link
      ...
      {surahStore?.surahList.find((s) => s.surahID === number) ? (
        <Icons.FilledHeart
          onClick={() =>
            surahStore?.toggleSurah(
              lang === "ar" ? name : englishNameTranslation,
              number
            )
          }
