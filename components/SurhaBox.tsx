    ...
    return (
        <div key={number} className=' flex w-full max-w-[300px] justify-between items-center p-4 py-6 bg-muted rounded-lg border hover:border-yellow-500 hover:bg-gradient-to-r hover:from-yellow-500/10 group'>
            <Link href={`surah/${number}`} className='flex items-center justify-between space-x-4 w-full'>
                ...
                <span className='group-hover:text-yellow-500'>{lang === "ar" ? `${numberOfAyahs} آيات` : `${numberOfAyahs} Ayahs`}</span>
            </Link>
            <Icons.Heart onClick={() => surahStore?.addToList(name, number)} className={cn("hover:text-red-500 hover:scale-125 transition-all duration-300 cursor-pointer", { "text-red-500 scale-125": surahStore?.surahList.find((s) => s.surahID === number) })} />
        </div>
    )
}
