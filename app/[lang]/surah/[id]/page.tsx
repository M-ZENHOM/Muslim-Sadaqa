...
export default async function page({ params, searchParams, params: { lang } }: { params: { id: number, lang: Locale }, searchParams: { [key: string]: string | string[] | undefined } }) {
    ...
    return (
        <Wrapper >
            <Card className='flex flex-col items-start md:items-center md:flex-col p-8 md:absolute md:left-4 md:space-y-10 md:h-[80vh] bg-muted '>
            ...
            <ScrollArea className="w-full h-[85vh] py-20">
            ...
            <audio src={`https://cdn.islamic.network/quran/audio/128/ar.alafasy/${ayahNum}.mp3`} autoPlay={ayahNum === 1 ? false : true} controls className=' w-full h-full min-h-[45px]' />
        </Wrapper>
    )
}
