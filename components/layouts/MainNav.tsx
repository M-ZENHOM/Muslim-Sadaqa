...
function MainNav({ NavMenu, lang }: { NavMenu: { title: string, href: string, IsNew?: boolean }[], lang: Locale }) {
    ...
    return (
        <nav className='w-[300px] h-screen p-6 space-y-5 hidden md:flex md:flex-col'>
            <ul className='flex flex-col space-y-3 text-lg font-extrabold border border-primary/50 rounded-lg p-6 bg-muted/25 bg-gradient-to-tr from-primary/25 to-50%'>
                {NavMenu.map((link, i) => (
                    <Link aria-label={link.title} key={i} className={cn('p-2 rounded-lg flex items-center transition-all duration-300 ', { "bg-primary text-primary-foreground": pathName === `/${lang}${link.href}` })} href={`/${lang}${link.href}`}>
